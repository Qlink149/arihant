#!/usr/bin/env python3
"""
cleanup_nurturing_task_piles.py
================================
Collapse duplicate pending Warm/Hot nurturing SLA follow-up piles.

For each lead with >1 open nurturing Warm/Hot follow-up task:
  - Keep the newest pending/in_progress task
  - Cancel the rest
  - Recompute next_action_date from remaining pending tasks

Usage
-----
  python backend/scripts/cleanup_nurturing_task_piles.py --dry-run
  python backend/scripts/cleanup_nurturing_task_piles.py --apply
"""
...
_NURTURING_DESCRIPTIONS = ("Hot Lead Follow-up", "Warm Lead Follow-up")
...
async def run(*, apply: bool) -> int:
    ...
    cursor = db.tasks.find(
        {
            ...
            "description": {"$in": list(_NURTURING_DESCRIPTIONS)},
    ...
    print(f"Leads with open nurturing Warm/Hot tasks: {len(by_lead)}")
    print(f"Leads with piles (>1 open task): {len(piles)}")
    ...
    for lead_id, tasks in piles.items():
        ...
        print(
            f"  lead={lead_id} open={len(tasks)} keep={keep.get('description')} "
            f"due={keep.get('due_date')} cancel={len(tasks) - 1}"
        )

    print(f"Would cancel {len(cancel_ids)} duplicate nurturing follow-up tasks")
        ...
        result = await db.tasks.update_many(
            {"id": {"$in": cancel_ids}},
            {
                "$set": {
                    ...
                    "completion_note": "Superseded by newer nurturing SLA task",
                }
            },
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Collapse duplicate Warm/Hot nurturing SLA follow-up task piles"
    )
