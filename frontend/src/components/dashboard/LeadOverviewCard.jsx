...
export const LeadOverviewCard = memo(function LeadOverviewCard({ metric, onClick }) {
  ...
  return (
    <button
      ...
      className="flex flex-col h-full text-left w-full bg-[#1A1A1A] border border-white/10 rounded-md p-3 card-hover hover:border-[#C5A059]/40 hover:bg-white/5 transition-colors cursor-pointer focus:outline-none focus-visible:ring-1 focus-visible:ring-[#C5A059]/50"
    >
      ...
      <p className="text-[10px] text-[#52525B] mt-auto pt-1">{subtitle}</p>
    </button>
  );
});
