...
export function LeadOverviewCard({ metric, onClick }) {
  ...
  return (
    <button
      ...
      className="flex flex-col h-full min-h-[120px] text-left w-full bg-[#141414] border border-white/[0.08] rounded-md p-4 hover:border-[#C5A059]/40 hover:bg-white/[0.02] transition-colors cursor-pointer focus:outline-none focus-visible:ring-1 focus-visible:ring-[#C5A059]/50"
    >
      <div className="flex items-start gap-2 mb-3">
        <span className={`w-1 h-6 shrink-0 rounded-sm ${bar}`} aria-hidden />
        <span
          className={`w-7 h-7 shrink-0 flex items-center justify-center border border-white/[0.06] rounded-sm bg-white/[0.02]`}
        >
          <Icon size={14} className={iconClass} aria-hidden />
        </span>
      </div>
      <p className="text-xs uppercase tracking-wide text-[#71717A] leading-tight">{label}</p>
